 ...
 * transport router hands a resolved `/mp-approve` command to,
...
export function createTodoistApprovalHandler(
  ...
  return {
    async onApprove(command: ApprovedMealPlanCommand): Promise<void> {
      ...
      if (!session) {
        logger.warn(
          `[todoist-commit] /mp-approve: no session row for week ${command.weekKey}; ignoring`,
        );
        return;
      }
      ...
      if (!plan) {
        logger.warn(
          `[todoist-commit] /mp-approve: no usable working_plan for week ${command.weekKey}; ignoring`,
        );
        return;
      }
